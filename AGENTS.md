...
- **`config.py`** — 唯一配置入口。可覆盖：`TOKENSTAT_GROK_LOG`、`TOKENSTAT_CLAUDE_MEM_CODEX_USAGE_DIR`（兼容旧名 `TOKENSTAT_CLAUDE_MEM_USAGE_DIR`）、`TOKENSTAT_CLAUDE_MEM_GROK_LOG`、`TOKENSTAT_OPENCLAW_AGENTS_DIR`、`TOKENSTAT_OPENCLAW_SESSION_DIR`、`TOKENSTAT_DATA_DIR`、`TOKENSTAT_HOST`/`PORT`/间隔/stale。HOST 空串或 `0.0.0.0` 回落到 `127.0.0.1`。
