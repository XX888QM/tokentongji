...
本机开机自启用 LaunchAgent `com.yunxin.tokenstat`（`RunAtLoad` + `KeepAlive`），与 hermes/openclaw 同一套路。launchd 子进程不能读 `~/Desktop`（实测 `open()` 报 `Operation not permitted`），所以 `scripts/install-launchd.sh` 会把 `src/` 和库拷到 `~/Library/Application Support/tokenstat/`，plist 只指向这份副本。改完仓库代码必须再跑一次安装脚本才会同步到自启进程。不要让 WorkingDirectory / PYTHONPATH 再指回桌面。装过自启后 `config.DATA_DIR` 默认也走 Application Support，避免手动启动写出第二套库。
