...
- **`config.py`** — 唯一配置入口。可覆盖：`TOKENSTAT_GROK_LOG`、`TOKENSTAT_CLAUDE_MEM_CODEX_USAGE_DIR`（兼容旧名 `TOKENSTAT_CLAUDE_MEM_USAGE_DIR`）、`TOKENSTAT_CLAUDE_MEM_GROK_LOG`、`TOKENSTAT_OPENCLAW_AGENTS_DIR`、`TOKENSTAT_OPENCLAW_SESSION_DIR`、`TOKENSTAT_CURSOR`/`TOKENSTAT_CURSOR_REFRESH`/`TOKENSTAT_CURSOR_STATE_DB`/`TOKENSTAT_CURSOR_CLI_CONFIG`、`TOKENSTAT_DATA_DIR`、`TOKENSTAT_HOST`/`PORT`/间隔/stale。HOST 空串或 `0.0.0.0` 回落到 `127.0.0.1`。
...
- **`parsers/{claude,codex,opencode,openclaw,hermes,grok,cursor}.py`** — 每来源一个模块，各自的去重键/差分逻辑是 recon 实测出来的坑（细节见各文件顶部 docstring），改动前务必先读：
  - Claude：普通消息用 `dedup_key = message.id`；同一 message 的流式重复行整体采用 total 更大的完整快照。fallback/retry 的 `usage.iterations` 必须拆成真实模型各一条，键统一为 `message.id:iteration:N`；看到 iterations 时删除此前的临时顶层 `message.id` 行，不能只读顶层 usage。1h/5m 缓存写入要分开计价。
