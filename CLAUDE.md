...
桌面端本地 Web 仪表盘，统计并汇总 Claude Code / Codex / OpenCode / OpenClaw / Hermes / Grok 六类 CLI/Agent 工具的 token 用量（按天/周/月/累计）。不维护手机端响应式布局。纯本地解析日志文件与 SQLite，Python 3.9+ 标准库即可运行，零第三方依赖（仅费用换算实时联网拉 USD→CNY 汇率）。
...
- **`parsers/{claude,codex,opencode,openclaw,hermes,grok}.py`** — 每来源一个模块，各自的去重键/差分逻辑是 recon 实测出来的坑（细节见各文件顶部 docstring），改动前务必先读：
  ...
  - OpenClaw：兼容 `*.trajectory.jsonl`（旧格式）与普通 `*.jsonl`（v3 session）两种格式；两套原始键不同，入库完成后按配对文件 + 时间/模型/token 指纹删除跨格式重复调用，保留 v3 行。
