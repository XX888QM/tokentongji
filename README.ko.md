...
| Codex | `~/.codex/sessions` + `archived_sessions` | 누적 `total_token_usage`의 인접 차이를 계산. fork된 subagent 파일의 첫 스냅샷은 기준값으로만 사용 |
...
- 오늘 / 최근 7일 / 이번 달 / 누적 token, CNY 예상 비용, 소스별 비중
- 중국식 큰 수 단위(万 / 亿 / 万亿 / 京 / 垓)와 마우스 오버 시 정확한 값
- 최근 30일 소스별 token 추이 차트
- 모델 / 프로젝트(cwd)별 token, 비용, cache token, 합계 및 기간 전환
...
## 로그인 시 자동 시작(선택 사항, macOS 전용)

```bash
# 현재 프로젝트 경로로 plist를 생성하고 로드
...
로그: `data/tokenstat.log` / `data/tokenstat.err.log`

설치 스크립트는 기본 포트와 `PYTHONPATH`만 기록하며 셸에서 export한 다른 `TOKENSTAT_*` 변수는 상속하지 않습니다. launchd에 사용자 설정이 필요하면 plist 템플릿을 수정한 뒤 다시 설치하세요. 일부 macOS 버전에서는 KeepAlive 재시작이 불안정하므로 필요할 때 포그라운드 실행이나 다른 프로세스 관리자를 사용하세요. 동일한 포트에 서비스를 중복 실행하지 마세요.
...
- 주소 사용 중 오류: 기존 수동 / launchd 서비스를 중지하거나 다른 `TOKENSTAT_PORT`를 설정하세요.
